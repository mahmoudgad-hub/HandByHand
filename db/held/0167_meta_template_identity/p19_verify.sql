...
--   4. AN EDIT KEEPS THE APPROVED NAME. The template Meta approved goes on
...
CALL hbh_test.chk_raises('state', 'DRAFT cannot jump to APPROVED - HB303',
  $q$ SELECT hbh.set_message_template_status((SELECT v FROM hbh_test.fx WHERE k='tpl'),
                                             'APPROVED', 'hbh_portal_update', 'ar') $q$, 'HB303');
CALL hbh_test.chk('state', 'DRAFT to SUBMITTED is accepted',
  $q$ SELECT hbh.set_message_template_status((SELECT v FROM hbh_test.fx WHERE k='tpl'), 'SUBMITTED')
             IS NOT NULL $q$);
CALL hbh_test.chk_raises('state', 'APPROVED with no name at all is refused - HB306',
  $q$ SELECT hbh.set_message_template_status((SELECT v FROM hbh_test.fx WHERE k='tpl'), 'APPROVED') $q$,
  'HB306');
-- A NAME META WOULD NOT RECOGNISE. Meta's template names are lower case;
-- typing this centre's own uppercase template_key here is the mistake that
-- is easiest to make and produces "template does not exist" at send time,
-- which reads as an approval that never happened.
CALL hbh_test.chk_raises('state', 'nor with a name Meta cannot have issued - HB306',
  $q$ SELECT hbh.set_message_template_status((SELECT v FROM hbh_test.fx WHERE k='tpl'),
                                             'APPROVED', 'PORTAL_UPDATE', 'ar') $q$, 'HB306');
-- AND THE LANGUAGE IS NOT OPTIONAL. Meta approves a template per language
-- and answers a name in a language it does not have exactly as it answers a
-- name it has never seen.
CALL hbh_test.chk_raises('state', 'nor with a name and no language - HB306',
  $q$ SELECT hbh.set_message_template_status((SELECT v FROM hbh_test.fx WHERE k='tpl'),
                                             'APPROVED', 'hbh_portal_update') $q$, 'HB306');
CALL hbh_test.chk('state', 'SUBMITTED to APPROVED with the name and language is accepted',
  $q$ SELECT hbh.set_message_template_status((SELECT v FROM hbh_test.fx WHERE k='tpl'),
                                             'APPROVED', 'hbh_portal_update', 'ar') IS NOT NULL $q$);
...
-- =====================================================================
-- CLAIM 4 - an edit keeps the approved name
-- =====================================================================
SET ROLE hbh_app;
RESET hbh.user_id;
-- THE NAME, THE LANGUAGE AND THE CATEGORY IN ONE ANSWER. A lookup that
-- handed back the name alone would send every message to a template Meta
-- answers for only in the language it was approved in, and an AUTHENTICATION
-- template one parameter short of the copy-code button it carries.
CALL hbh_test.chk('keep', 'the approved template is what a REQUEST_DECIDED message is sent under',
  $q$ SELECT template_name = 'hbh_portal_update' AND language_code = 'ar' AND NOT is_auth
      FROM hbh.message_template_ref((SELECT v FROM hbh_test.fx WHERE k='center'), 'REQUEST_DECIDED') $q$);
...
CALL hbh_test.chk('keep', 'which puts it back in DRAFT and keeps the name and language',
  $q$ SELECT status = 'DRAFT' AND template_name = 'hbh_portal_update' AND language_code = 'ar'
      FROM hbh.message_templates WHERE template_id = (SELECT v FROM hbh_test.fx WHERE k='tpl') $q$);

RESET hbh.user_id;
CALL hbh_test.chk('keep', 'so the approved template is still the one sent',
  $q$ SELECT template_name = 'hbh_portal_update'
      FROM hbh.message_template_ref((SELECT v FROM hbh_test.fx WHERE k='center'), 'APPOINTMENT_CANCELLED') $q$);
CALL hbh_test.chk('keep', 'and a code with no approved template in this centre has none',
  $q$ SELECT template_name IS NULL
      FROM hbh.message_template_ref((SELECT v FROM hbh_test.fx WHERE k='center'), 'OTP_LOGIN') $q$);
-- AND THAT ROW IS STILL AN ANSWER, not silence: the login template exists and
-- says it is an authentication one, which is how the transport knows to send
-- the code twice the day it is approved.
CALL hbh_test.chk('keep', 'the unapproved login template still reports its category',
  $q$ SELECT is_auth
      FROM hbh.message_template_ref((SELECT v FROM hbh_test.fx WHERE k='center'), 'OTP_LOGIN') $q$);
-- A code no centre has a template for answers no row at all, which is what
-- centres_without_approved_template counts.
CALL hbh_test.chk('keep', 'a code with no template at all answers nothing',
  $q$ SELECT count(*) = 0
      FROM hbh.message_template_ref((SELECT v FROM hbh_test.fx WHERE k='center'), 'NO_SUCH_CODE') $q$);
CALL hbh_test.chk('keep', 'and this centre is counted among those that cannot send a login code',
  $q$ SELECT hbh.centres_without_approved_template('OTP_LOGIN') >= 1 $q$);

-- =====================================================================
-- CLAIM 5 - the history, and the worker's lookup
-- =====================================================================
RESET ROLE;
CALL hbh_test.chk('history', 'every change was recorded, with the text and name as they became',
  $q$ SELECT array_agg(to_status ORDER BY event_id) = ARRAY['DRAFT','DRAFT','SUBMITTED','APPROVED','DRAFT']
         AND (SELECT template_name || ' ' || language_code FROM hbh.message_template_events
               WHERE template_id = (SELECT v FROM hbh_test.fx WHERE k='tpl') AND to_status = 'APPROVED')
             = 'hbh_portal_update ar'
...
CALL hbh_test.chk_raises('worker', 'a user session cannot use the worker''s lookup - HB230',
  $q$ SELECT template_name FROM hbh.sms_template_ref((SELECT v FROM hbh_test.fx WHERE k='sms')) $q$,
  'HB230');
RESET hbh.user_id;
CALL hbh_test.chk('worker', 'the worker gets the centre''s approved template for a claimed message',
  $q$ SELECT template_name = 'hbh_portal_update' AND language_code = 'ar' AND NOT is_auth
      FROM hbh.sms_template_ref((SELECT v FROM hbh_test.fx WHERE k='sms')) $q$);
RESET ROLE;
