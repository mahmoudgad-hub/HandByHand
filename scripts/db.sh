...
cmd_verify() {
  ...
  # THE PHASE NAME IS CHECKED BEFORE wait_healthy, because it is a filename
  # question and not a database one - and a name that runs nothing must not
  # wait for anything. `verify p00` (the phase is "00") once matched no
  # suite, skipped every one, and reported success. The guard for that has
  # to fire in a second with the list of real names; if it sat after
  # wait_healthy it would first block on a container start - or, when the
  # docker engine is down, die on the wait - only to refuse a typo anyway.
  local matched=0 avail=""
  for f in "${suites[@]}"; do
    [ -e "$f" ] || continue
    phase="$(basename "$f" | sed 's/^p\([0-9a-z]*\)_verify\.sql$/\1/')"
    avail="$avail $phase"
    if [ -z "$want" ] || [ "$want" = "$phase" ]; then matched=1; fi
  done
